...
namespace ModuleStocks.ViewModels
{
    ...
    {
        private Brush _increaseBrush, _reducedBrush;

        private int _lastAddNewIndex = -1;

        private PropertyDescriptorCollection _pdc;

        private IBindingList _sourceList;
        ...
        #endregion SourceList, Wire/Unwire, Get Record Count
        ...
        #endregion Bind field values from List to Cells

        #region ColumnChanging Event

        // Allow this table to listen to DataTable.ColumnChanging events. When a ColumnChanging event is handled the table will automatically add ChangedFieldInfo objects with
        // information about the new and old value of the column. The ChangedFieldInfo objects will then be checked in the ListChanged event handler.

        private bool _dataSourceRaisesTwoItemAddedEvents;

        private DataTable _dt;

        /// <summary>A helper method that calls pd.GetValue(row) or gets the value directly from a DataRow using its name.</summary>
        ...
        public static object GetValue(object row, PropertyDescriptor pd)
        {
            if (row is DataRow)
            {
                // PropertyDescriptor is for the DataRowView - but here we only have access to the DataRow.
        ...
        /// <summary>This virtual method is called when a row was added in the underlying datasource.</summary>
        /// <param name="row"></param>
        protected virtual void OnPrepareItemAdded(object row)
        {
            ...
            // foreach (string name in this.totalSummaries) { FieldDescriptor fd = td.Fields[name]; if (fd.IsPropertyField()) { PropertyDescriptor pd =
            // fd.GetPropertyDescriptor(); object value = GetValue(row, pd);
        ...
        /// <summary>This virtual method gets called before a row is removed from the underlying datasource.</summary>
        /// <param name="row"></param>
        ...
        private DataTable GetDataTable(object datasource)
        {
        ...
        private void PrepareRemoving(object row)
        {
        ...
        #endregion ColumnChanging Event

        #region Changed Fields

        private Hashtable _changedFields = new Hashtable();

        private ChangedFieldInfoCollection _changedFieldsArray = new ChangedFieldInfoCollection();

        /// <summary>The Collection with detected changes in the datasource when a ListChanged event is handled.</summary>
        ...
        /// <summary>
        /// Call this method to add ChangedFieldInfo objects with information about the new and old value of the column. The ChangedFieldInfo objects will then be checked in
        /// the LIstChanged event handler.
        /// </summary>
        ...
        private void SourceList_ListChanged(object sender, ListChangedEventArgs e)
        {
            switch (e.ListChangedType)
            {
                case ListChangedType.ItemChanged:
                    ...
                    // DataView raises a ItemAdded with subsequent ItemChanged and ItemMoved event when data view is sorted Skip the ItemChanged notification in such case.
                ...
                case ListChangedType.ItemAdded:
                    ...
                    // DataView raises a ItemAdded with subsequent ItemMoved event when data view is sorted. The lastAddNewIndex will be checked in the OnSourceListItemChanged
                    // and ItemMoved handler so that the subsequents events can be ignored.
                ...
                case ListChangedType.ItemMoved:
                    ...
                    // DataView raises a ItemAdded with subsequent ItemChanged and ItemMoved event when data view is sorted In the ItemAdded event the e.NewIndex already had
                    // the correct index and was inserted at the right position. Doing it here again would mess up order. Therefore skip the ItemMoved notification.
        ...
        #endregion IBindingList.ListChanged handler
        ...
        private class BlinkTable : RowColumnIndexValueDictionary<BlinkInfo>, IRowColumnIndexValueDictionaryCallbacks<BlinkInfo>
        {
            public BlinkTable()
            {
                SetCallback(this);
            ...
            #endregion IRowColumnIndexValueCallbacks<BlinkInfo> Members
        }

        private List<BlinkInfo> _blinkQueue = new List<BlinkInfo>();

        private BlinkTable _blinkTable = new BlinkTable();

        private int _blinkTime = 700;

        private int _tickCountAtLastProcessBlinkQueue;
        ...
        private void BlinkQueueDeleteField(int index, BlinkTable moveBlinks)
        {
        ...
        private void BlinkQueueInsertField(int index, BlinkTable moveBlinks)
        {
        ...
        private void ProcessBlinkQueue()
        {
        ...
        private bool _invalidateWholeSummaryRow = false;

        private List<double> _totals = new List<double>();
